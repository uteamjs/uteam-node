...
class sqlObject {
    ...
    updateLogTable = (pid, table, action) => cb => {
        if (this.dbtype == 1) {
            const [setting, param] = this._fieldValueParam({
                pid, table,
                uid: this.req.user.uid,
                action, timestamp: 'NOW()'
            })
            this.queryParam(`insert into ${table} updates ${setting}`, param)(cb)

        } else
            this.query(`insert into ${table} updates ` + this._fieldValue({
                pid, table,
                uid: this.req.user.uid,
                action, timestamp: 'NOW()'
            }))(cb)
