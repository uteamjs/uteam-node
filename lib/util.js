...
const colors = process.env.colorMono ? false : require('colors')
...
const _getModuleFile = s => {
    ...
    return colors ? colors.grey(txt) : txt
}
...
function callLog(tp, argv) {
    ...
    argv[0] = ('linux|darwin'.indexOf(process.platform) >= 0 ? linuxCaller() : windowCaller(2 + n))
        + (colors ? colors[_getColor[tp] || 'reset']('\n>> ' + s) : '\n>> ' + s)
