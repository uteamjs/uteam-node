					...
					(error, user, info) => {
						...
						if (user === undefined || error !== null) {
							...
							return res.status(401).json({
								message: error?.message ?? 'Please login again, session expired!'
							})
						}
						else {
							//Extend Token
							const _token = jwt.decode(req.get('token'), p.buffer)
							delete _token.exp

							res.setHeader("Access-Control-Expose-Headers", 'token')
							res.setHeader('token', jwt.sign(_token, p.secret, p.options))

							if (req.url === '/api') 
								req.session.uid = _token.uid

							return req.url === '/validatesession' ? res.status(200).json({}) : next()
						}
					})(req, res, next)

			// Excluded url or Passport authenticated with session 
			else if (_exclude(req.url) || req.session?.passport?.user)
				return next()
